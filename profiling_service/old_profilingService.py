# -*- coding: utf-8 -*-
from flask import Flask, render_template, request, redirect, session, json, flash, url_for
...
def init_db():
    ...
    #
    alert1 = Alert(User.objects.get_or_404(email='j.doe@gmail.com'))
    alert1.save()
...
def amberalert(user_email):
    ...
    volunteers = User.objects(user_type='volunteer',
                              position__near=[user.position['coordinates'][0], user.position['coordinates'][1]],
                              position__max_distance=10000)
...
@app.route('/service/set_psap', methods=['POST'])
def set_psap():
    data = request.data
    dataDict = json.loads(data)
    user = User.objects(email=dataDict['user']).first()
    user.update(set__serving_psap=dataDict['psap'])
